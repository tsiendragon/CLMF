...
| **Photo Response Non-Uniformity (PRNU) Noise**    | Unique sensor noise “fingerprint” left by a camera’s imaging sensor; real photos have consistent sensor noise, while deepfakes disrupt or lack this pattern.                                                                                                                                                                                                        | Compute the high-frequency noise residual from the image (e.g. via wavelet or high-pass filtering) and compare against expected camera PRNU pattern or across regions. Inconsistencies in the face region’s noise indicate manipulation.                                                                                   | Early work by Peng *et al.* (2019) used PRNU analysis to spot altered sensor noise in fake face inserts. Also used in forensic methods to detect inconsistencies when a face is transplanted onto a background.                                                                                                                                                                                                                          |
